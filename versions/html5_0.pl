...
#
### File: html5.pl
### Language definitions for HTML 5
...
#    }
	...
	$return = join('', (($cap_anchors)? "$cap_anchors\n" : '')
		, "<TABLE$env_id$lang class=\"PAD $border\" style=\""
		, $tab_width, "\">");
...
sub do_env_equation {
    ...
    # include the equation-number, using a <DIV class="equation">
    local($halign) = " &nbsp; " unless $FLUSH_EQN;
    local($lang) = &get_current_language();
    if ($EQN_TAGS =~ /L/) {
	...
	$_ = join ('', $comment, $math_start
		   , &process_math_in_latex("indisplay", '', '', $saved)
		   , $math_end );
    } else {
	s/^[ \t]*\n?/\n/; s/\n?[ \t]*$/\n/;
